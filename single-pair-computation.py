...
norm = np.linalg.norm
def r_vec(iv): return iv[0]*ao + iv[1]*a1 + iv[2]*a2 + iv[3]*a3
def hat(v): return v/norm(v)

def A(iv):
    r = r_vec(iv)
    return norm( ge*gC13/(4*np.pi*(norm(r)*a0)**3) *
                 (zhat-3*np.dot(hat(r),zhat)*hat(r)) )

def z_projection_sum(iv):
    return abs(3*iv[0] + 4*(iv[1]+iv[2]+iv[3]))
def xy_projection_sum(iv):
    return 3*(iv[1]**2+iv[2]**2+iv[3]**2) - (iv[1]+iv[2]+iv[3])**2
...
probability = 1 - product([ 1 - c13_abundance**2 * (1-c13_abundance)**(R-2) *
                            ...
                            for R in [len(larmor_set)]
                            for pss in [parallel_subset_sizes(larmor_set)] ])
print(probability)
