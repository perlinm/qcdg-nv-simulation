...
of the form
\begin{align}
  H_\t{int} = h_\t{int} I_\NV^z\v I_j\c\uv n_j,
  \label{H_int}
\end{align}
for a tunable coupling strength $h_\t{int}$ and direction $\uv n_j$,
where $I_\NV^z\equiv\p{\op{m_s}-\op{0}}/2$ and $\v I_j$ is the spin
...
(\ref{H_int}) will simultaneously couple $I_\NV^z$ to both
...
fact that ${\cNOT^{j\to k}}^\dagger=\cNOT^{j\to k}$. Therefore,
\begin{align}
  \SWAP^{\NV,\AC}
  =& \exp\p{-j\f\pi2 I_\NV^x} \exp\p{j\f\pi2 I_1^{z'}} \exp\p{-j\pi
     I_\NV^x I_1^{z'}}
     \times \tag*{} \\
   & \exp\p{-j\f\pi2 I_\NV^z} \exp\p{-j\f\pi2 I_1^{x'}} \exp\p{j\pi
     I_\NV^z I_1^{x'}}
     \times \tag*{} \\
   & \exp\p{-j\f\pi2 I_\NV^z} \exp\p{-j\f\pi2 I_2^{x''}} \exp\p{j\pi
     I_\NV^zI_2^{x''}}
     \times \tag*{} \\
   & \exp\p{-j\f\pi2 I_\NV^x} \exp\p{-j\f\pi2 I_1^{z'}} \exp\p{j\pi
     I_\NV^x I_1^{z'}},
...
\begin{align}
  \SWAP^{\NV,\AC}
  =& \exp\p{j\f\pi2 I_\NV^x} \exp\p{-j\pi I_\NV^x I_1^{z'}}
     \times \tag*{} \\
   & \exp\p{-j\pi I_\NV^z} \exp\p{j\f\pi2\sp{I_1^{y'}-I_2^{x''}}}
     \times \tag*{} \\
   & \exp\p{-j\pi I_\NV^z I_1^{y'}} \exp\p{j\pi I_\NV^z I_2^{x''}}
     \times \tag*{} \\
...
\begin{align}
  ...
  =& \exp\p{j\f\pi2 I_\NV^x} \exp\p{-j\pi I_\NV^x I_1^z}
   ...
   & \exp\p{-j\pi I_\NV^z} \exp\p{j\f\pi2\sum_jI_j^{y'}}
     \times \tag*{} \\
   & \exp\p{-j\pi I_\NV^z I_1^{y'}} \exp\p{-j\pi I_\NV^z I_2{y'}}
     \times \tag*{} \\
   & \exp\p{j\pi I_\NV^x I_1^z} \exp\p{-j\f\pi2 I_\NV^x}.
...
\begin{align}
  ...
  =& \exp\p{-j\f\pi2 I_\NV^y} \exp\p{j\f\pi2 I_1^{y'}}
     \times \tag*{} \\
   & \exp\p{-j\pi I_\NV^z I_1^{x'}} \times \exp\p{-j\f\pi2 I_1^{y'}}
     \exp\p{j\f\pi2 I_\NV^y}.
\end{align}
Substituting this identity into (\ref{S_SWAP_NVAC_sxsz}) yields
\begin{align}
  \SWAP^{\NV,\AC} =& \exp\p{j\f\pi2 I_\NV^x} \exp\p{-j\f\pi2 I_\NV^y}
  \times \tag*{} \\
  & \exp\p{j\f\pi2\sum_jI_j^{y'}} \exp\p{-j\pi I_\NV^z I_1^{x'}}
  \exp\p{j\f\pi2 I_\NV^y}
  \times \tag*{} \\
  & \exp\p{-j\pi I_\NV^z} \exp\p{j\f\pi2\sum_jI_j^{y'}}
  \times \tag*{} \\
  & \exp\p{-j\pi I_\NV^z I_1^{y'}} \exp\p{-j\pi I_\NV^z I_2^{y'}}
  \times \tag*{} \\
  & \exp\p{-j\f\pi2 I_\NV^y} \exp\p{j\pi I_\NV^z I_1^{x'}}
