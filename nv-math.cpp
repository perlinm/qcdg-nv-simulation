...
MatrixXcd U_int(const nv_system& nv, const uint index, const uint k_DD,
                const Vector3d& axis_NV, const double target_axis_azimuth, const double phi,
  ...
  MatrixXcd U = U_flush*U_interaction;
  ...
  const Matrix2cd sxp = dot(s_vec,target_xhat);
  const Matrix2cd syp = dot(s_vec,target_yhat);
  const Matrix2cd sn = cos(target_axis_azimuth)*sxp + sin(target_axis_azimuth)*syp;

  // desired propagator
  MatrixXcd U_desired = exp(-j*phi * act(tp(sz,sn), {0,index_in_cluster+1}, spins));
