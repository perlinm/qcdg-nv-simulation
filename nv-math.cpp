...
// realization of propagator U = exp(-i * rotation_angle * sigma_{axis}^{index})
...
// propagator generated by t_0 H = rotation_angle * sigma_{n_1}^{NV} * sigma_{n_2}^{index}
...
// compute fidelity of nuclear spin control operation
double control_fidelity(const nv_system& nv, const uint index,
                        const double target_axis_azimuth, const double rotation_angle){
  // realized control operation
  ...
  const MatrixXcd exact_gate_cluster = act(exact_gate, {index_in_cluster+1}, spins);

  return gate_fidelity(U, exact_gate_cluster);
}
...
double coupling_fidelity(const nv_system& nv, const uint index, const uint k_DD,
  ...
  const uint spins = nv.clusters.at(get_cluster_containing_index(nv,index)).size()+1;
  const uint index_in_cluster = get_index_in_cluster(nv,index);
  const MatrixXcd exact_gate = exp(-j * rotation_angle * tp(sn_nv,sn_target));
  const MatrixXcd exact_gate_cluster = act(exact_gate, {0,index_in_cluster+1}, spins);
