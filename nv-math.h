...
inline double w_NV_GS(const nv_system& nv, const Vector3d B_ctl = Vector3d::Zero()){
  return nv.ms*NV_ZFS - nv.e.g*(nv.static_Bz + dot(B_ctl,zhat));
};

// propagator generated by t_0 H = phi * sigma_n^{NV}
inline MatrixXcd U_NV(const Vector3d axis, const double phi, const uint spins){
  return act( exp(-j*phi*dot(s_vec,hat(axis))), {0}, spins);
}
